...
from ..manopth.tensutils import subtract_flat_id, th_posemap_axisang, th_with_zeros
...
class ManoLayer(Module):
    ...
    def forward(
        ...
        if self.use_pca or self.joint_rot_mode == "axisang":
            ...
            assert th_pose_coeffs.shape[2:4] == (
                3,
                3,
            ), "When not self.use_pca, th_pose_coeffs have 3x3 matrix for two" "last dims, got {}".format(
        ...
        if th_betas is None or th_betas.numel() == 1:
            ...
            th_v_shaped = (
                torch.matmul(self.th_shapedirs, th_betas.transpose(1, 0)).permute(2, 0, 1) + self.th_v_template
            )
